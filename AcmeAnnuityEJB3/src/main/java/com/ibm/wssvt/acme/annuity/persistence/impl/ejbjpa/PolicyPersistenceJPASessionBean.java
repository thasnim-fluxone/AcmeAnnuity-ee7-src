...
import com.ibm.wssvt.acme.annuity.common.bean.jpa.Beneficiary;
...
public class PolicyPersistenceJPASessionBean implements PolicyPersistence<IPersisteble<?>, String, String>{
	...
	public List<IPolicy> getHolderPolicies(IAnnuityHolder holder) throws InvalidArgumentException, ServerPersistenceModuleException {
		...
		logger.fine("getHolderPoicies. Holder info: id: " + holder.getId());
		...
		try{		 						
			...
			else if (queryType != null && queryType.toLowerCase().equals("stored")) {
				logger.fine("Performing Named Stored Procedure query for getHolderPolicies");
				
				// Rumana added for JPA21 - to test Named Stored Procedure Query
				StoredProcedureQuery spq = em.createNamedStoredProcedureQuery("GetPolicyByHolderId");
				
				//StoredProcedureQuery spq = em.createStoredProcedureQuery("EJB30.HOLDER_POLICY", Policy.class);
				//spq.registerStoredProcedureParameter("holderId_1", String.class, ParameterMode.IN);
				
				
				spq.setParameter("holderId_1",holder.getId());
	            								
				List<IPolicy> polList = spq.getResultList();
	...
	public List<IBeneficiary> getHolderBeneficiaries(IAnnuityHolder holder) throws InvalidArgumentException, ServerPersistenceModuleException {
		...
		try{
			...
			if(queryType != null && queryType.toLowerCase().equals("criteria")) {
				...
				logger.fine("Performing Stored Procedure query for getHolderBeneficiaries");
				// Rumana added for JPA21
				StoredProcedureQuery spq = em.createStoredProcedureQuery("EJB30.HOLDER_BENEFICIARY", Beneficiary.class);
				spq.registerStoredProcedureParameter("holderId_2", String.class, ParameterMode.IN);
				
				//Workaround for defect 
				//spq.registerStoredProcedureParameter(0, String.class, ParameterMode.IN);
				//spq.setParameter(0, holder.getId());
								
				spq.setParameter("holderId_2",holder.getId());
				List<IBeneficiary> benList = spq.getResultList();
