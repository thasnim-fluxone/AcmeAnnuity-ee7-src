...
@RunAs("admins")
@DeclareRoles("admins")
public class AcmeAnnuityMgmtSvcEJB30MDBBean implements MessageListener {

	@Resource(name = "jms/ACMEAnnuityJMSConnectionFactory")
	private ConnectionFactory jmsConnFactory;
	
	//System.out.println("JMS injection is " + jmsConnFactory);
	...
	public void onMessage(Message message) {       
    	...
    	try {
			msg = validateInputMessage(message);
			
			// JMS 2.0:  need figure out when to call validateInputMessageJMS20 if JMS2.0 is used
			// since this is onMessage(), how do we read from client cfg file
    	...
    	try {    	    		
    		annuityJMSMessage = (AnnuityJMSMessage) msg.getObject(); 
    		
    		// in JMS2.0, get the return message from validateInputMessageJMS20()
    		// annuityJMSMessage = message.getBody(AnnuityJMSMessage.class);
    		
    		logger = getLogger(annuityJMSMessage.getInput());
    		logger.fine("inside the onMessage of the MDB. message: " + message);    		   	    		
    		...
    		sendReply(replyDest, annuityJMSMessage, logger);
	...
	private void sendReply(Destination replyDest,
		...
		try {
			...
			Session jmsReplySession = connection.createSession(false, Session.AUTO_ACKNOWLEDGE);	       
			MessageProducer queueSender = jmsReplySession.createProducer(replyDest);
			ObjectMessage replyMessage = jmsReplySession.createObjectMessage();            
			replyMessage.setObject(annuityJMSMessage);			
			queueSender.send(replyMessage);			
		} catch (JMSException e) {
			// not much i can do!  I cannot send the message back to the client
			if (logger != null) {
				Logger.getLogger(getClass().getName()).warning("Failed to send JMS reply. JMSException error. Error: " + ExceptionFormatter.deepFormatToString(e));
				logger.log(Level.WARNING, "Faield to send JMS reply. JMSException error. Error: " + ExceptionFormatter.deepFormatToString(e), e);
				e.printStackTrace();
			}else{			
				Logger.getLogger(getClass().getName()).warning("Failed to send JMS reply. JMSException error. Error: " + ExceptionFormatter.deepFormatToString(e));				
				e.printStackTrace();
			...
			Logger.getLogger(getClass().getName()).warning(
					"The JMS sendReply() method failed with this exception - " + ExceptionFormatter.deepFormatToString(e));
			e.printStackTrace();
		} catch (Throwable t){		    
			Logger.getLogger(getClass().getName()).warning(
					"The JMS sendReply() method failed with this throwable - " 
			...
			if (connection != null) {
				try {
					...
					Logger.getLogger(getClass().getName()).warning(
					    "The JMS sendReply() method failed when closing the connection with this exception - " + ExceptionFormatter.deepFormatToString(e));
					e.printStackTrace();
				}
			}
		}
	...
	private void sendReplyJMS20(Destination replyDest,
		...
		try (JMSContext context = jmsConnFactory.createContext();){
			...
			if (logger != null) {
				...
				logger.log(Level.WARNING, "Faield to send JMS reply. JMSException error. Error: " + ExceptionFormatter.deepFormatToString(ex), ex);
				ex.printStackTrace();
			}else{			
				Logger.getLogger(getClass().getName()).warning("Failed to send JMS reply. JMSException error. Error: " + ExceptionFormatter.deepFormatToString(ex));				
				ex.printStackTrace();
	...
	private ObjectMessage validateInputMessage(Message message) throws JMSException, ServerBusinessModuleException {
		ObjectMessage msg;
		if (message == null ){
			throw new ServerBusinessModuleException ("The JMS parameter message is invalid. Its null.");
		}
		...
		if (!(msg.getObject() instanceof AnnuityJMSMessage)){
			throw new ServerBusinessModuleException ("The JMS parameter message contains invalid object." +
					"The internal object must be of type AnnuityJMSMessage " +
					" found class is: " + msg.getObject().getClass());
		}
		
		AnnuityJMSMessage annuityJMSMessage = (AnnuityJMSMessage) msg.getObject();
		...
		if (message.getJMSReplyTo() == null) {
			throw new ServerBusinessModuleException ("The JMS Reply is not valid. Its null.  Its expected that each operation to have provided a JMS Reply.");
		}	
		return msg;
	}
