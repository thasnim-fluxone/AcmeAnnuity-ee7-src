...
/**
 * This execution unit tests that an annuity object can be updated and persisted
 * correctly by JPA.  The test gets a holder and from the holder gets a list of
 * all the holder's annuities.  It then picks one annuity at a time and tries the
 * following.  Adds payors, updates an added payor, removes the added payors.
 * Adds payouts, updates an added payout, removes the added payouts.  Adds riders,
 * updates an added rider, and removes the added riders.  After each add, update, or
 * remove the annuity is checked to make sure it has been persisted correctly.
 * @author Jeff
 *
 */
public class UpdateAnnuityWithAllChildrenEU extends AbastractAnnuityExecutionUnit {
				...
				while(threshold <= maximumThreshold ){
					try {
						...
						else
						{
							...
							{
								logger.severe("After an optomistic locking exception or deadlock on an annuity, an exception was thrown while trying to retrieve the persisted annuity");
...
//				Make sure the annuity currently being used is the most up to date one.
			 ...
			 */
			logger.fine("Attempting to add payouts to annuity");
			ArrayList<String> payouts = new ArrayList<String>();
			
				while(threshold <= maximumThreshold ){
					try {
						payouts = new ArrayList<String>();
	...
	ServerInternalErrorException, RemoteException {
		...
		annuity.setConfiguration(getConfiguration());
		return (IEquityAnnuity) getServerAdapter().updateAnnuity(annuity);
	}
	...
	ServerInternalErrorException, RemoteException {
		...
		annuity.setConfiguration(getConfiguration());
		return (IFixedAnnuity) getServerAdapter().updateAnnuity(annuity);
	}
	...
	private IPayor getAnnuityPayor(IAnnuity annuity, String payorId)
	{
		...
		{
			if(annuity.getPayors().get(i).getId().equals(payorId))
			{
	...
	private IAnnuity removePayorFromAnnuity(IAnnuity annuity, String payorId) throws EntityNotFoundException, InvalidArgumentException, ServerAdapterCommunicationException, ServerInternalErrorException, ExecutionUnitVerificationException, RemoteException {
		IAnnuity annuityBug;
		annuity.setConfiguration(getConfiguration());
		annuityBug = getServerAdapter().findAnnuityById(annuity);
		...
		IPayor payor = getAnnuityPayor(annuity, payorId);
		
		if(removeAnnuityPayorHelp(annuity, payorId) == false)
		{
	...
	private IAnnuity addPayorToAnnuity(IAnnuity annuity, ArrayList<String> payors) throws EntityAlreadyExistsException, InvalidArgumentException, ServerAdapterCommunicationException, ServerInternalErrorException, EntityNotFoundException, RemoteException  {
		...
		payors.add(payor1.getId());
		payors.add(payor2.getId());
		annuity.setConfiguration(getConfiguration());
		return getServerAdapter().updateAnnuity(annuity);
	}
	...
	private IAnnuity removeAnnuityPayout(IAnnuity annuity, String payoutId) throws EntityNotFoundException, InvalidArgumentException, ServerAdapterCommunicationException, ServerInternalErrorException, ExecutionUnitVerificationException, RemoteException {	
		...
		logger.fine("REMOVING PAYOUT = " + payoutId);
		...
		logger.fine("REMOVING PAYOUT = " + payout.getId());
		return resultAnnuity;
	}
	...
	private IAnnuity addPayoutsToAnnuity(IAnnuity annuity, ArrayList<String> payouts) throws EntityAlreadyExistsException, InvalidArgumentException, ServerAdapterCommunicationException, ServerInternalErrorException, EntityNotFoundException, ExecutionUnitVerificationException, RemoteException {		
		...
		logger.fine("ADDED PAYOUT = " + payout1.getId());
		logger.fine("ADDED PAYOUT = " + payout2.getId());
		
		
		logger.fine("Start Output All payout IDS");
		for(IPayout temp : annuity.getPayouts())
		{
			logger.fine(temp.getId());
		}
		...
		annuity.setConfiguration(getConfiguration());
		IAnnuity toRet = getServerAdapter().updateAnnuity(annuity);
	...
	*/
	...
	{
		logger.fine("annuity payouts size = " + annuity.getPayouts().size());
		for(IPayout temp : annuity.getPayouts())
		{
			logger.fine(temp.getId());
		}
		logger.fine("results payouts size = " + results.getPayouts().size());
