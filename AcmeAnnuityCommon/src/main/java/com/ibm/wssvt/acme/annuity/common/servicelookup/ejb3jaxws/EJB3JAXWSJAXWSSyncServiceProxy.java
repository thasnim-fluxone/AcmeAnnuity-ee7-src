...
public class EJB3JAXWSJAXWSSyncServiceProxy implements IAnnuityService {

	private AnnuityMgmtSvcEJB30JAXWS webService;

	public EJB3JAXWSJAXWSSyncServiceProxy(
			AnnuityMgmtSvcEJB30JAXWS webServiceInterface) {
		this.webService = webServiceInterface;
	}

	private void processCommonExceptions(Exception e)
			throws InvalidArgumentException, ServerBusinessModuleException,
			ServerPersistenceModuleException {
	...
	public List<IAnnuity> findPayorAnnuities(IPayor payor)
		...
		try {
			List<AnnuityValueObject> tempList = webService
