...
public class EJB31AsynchServiceLookup {

	public static IAnnuityService getAnnuityEJB31Service(AnnuityMgmtAsynchSvcEJB31 injectedRemote,
			AnnuityMgmtAsynchSvcEJB31Local injectedLocal,	Configrable<String, String> configrable, AcmeLogger logger) 
	throws InvalidConfigurationException, NamingException, AnnuitySecurityException {
		...
		if ("injectionWithLocal".equalsIgnoreCase(mode)) {
			return new AnnuityEJB31AsynchLocalServiceProxy(injectedLocal);
		}else if ("local".equalsIgnoreCase(mode)) {
			return new AnnuityEJB31AsynchLocalServiceProxy(getAnnuityEJB31LocalService(configrable, logger));
		}else if ("remote".equalsIgnoreCase(mode)){
			return new AnnuityEJB31AsynchRemoteServiceProxy (getAnnuityEJB31RemoteService(configrable, logger), logger);
		}else if ("injectionWithRemote".equalsIgnoreCase(mode)){
			return new AnnuityEJB31AsynchRemoteServiceProxy (injectedRemote, logger);
		}else{			
	...
	public static AnnuityMgmtAsynchSvcEJB31 getAnnuityEJB31RemoteService(Configrable<String, String> configrable, AcmeLogger logger) throws NamingException, AnnuitySecurityException  {
...
//			}														
			...
			svcRemote = (AnnuityMgmtAsynchSvcEJB31) PortableRemoteObject.narrow(ref, AnnuityMgmtAsynchSvcEJB31.class);
	...
	public static AnnuityMgmtAsynchSvcEJB31Local getAnnuityEJB31LocalService(Configrable<String, String> configrable, AcmeLogger logger) throws NamingException  {
		AnnuityMgmtAsynchSvcEJB31Local svc  = null;	
		try {						
			InitialContext ic = new InitialContext();
			logger.fine("obtained initial context: " + ic);
			Object ref = ic.lookup(configrable.getConfiguration().getParameterValue("ejb31LocalJNDI"));
			svc = (AnnuityMgmtAsynchSvcEJB31Local) ref; 
