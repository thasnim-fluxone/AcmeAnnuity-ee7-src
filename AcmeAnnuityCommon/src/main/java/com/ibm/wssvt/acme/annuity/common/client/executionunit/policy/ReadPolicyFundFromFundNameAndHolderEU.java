...
public class ReadPolicyFundFromFundNameAndHolderEU extends AbastractPolicyExecutionUnit {
	 ...
	 * 4 -For each of the policies found - get the first Fund.
	 * 5 - Query the fund based on the holderId and FundName
	 * 6 - Get the fundIndexRate
	 * 7 - Verify the Fund returned in the same using both the queries
	 */
		...
		if (policyListResults != null) {
			...
			while (it.hasNext()) {
				...
				/*	for(int i=1; i <= pol.getFunds().size(); i++) {
						String fundName = pol.getFunds().get(new Integer(i)).getFundName();
						logger.fine("Policy's fund name is: " + fundName);
						logger.fine("Looking up Funds for holder ID: " + policyHolder.getId());
						IFund fund = getServerAdapter().findFundIndexRateFromFundName(policyHolder, fundName);
						logger.fine("Fund's Name is: "+fund.getFundName()); 
						logger.fine("Fund's index rate is: "+fund.getIndexRate()); 
				...
				//Find the fundIndexRate for the 1st fund in each policy.
				
				try {
					fundQuery1 = pol.getFunds().get(new Integer(1));
					//String fundName = pol.getFunds().get(new Integer(1)).getFundName();
					String fundName = fundQuery1.getFundName();
					if (fundName != null) {
						logger.fine("Looking up Fund for holder ID: " + policyHolder.getId()+ " with FundName "+ fundName);
						fundQuery2 = getServerAdapter().findFundIndexRateFromFundName(policyHolder, fundName);
						logger.fine("The IndexRate for Fund " + fundName + " is : " + fundQuery2.getIndexRate());
					}
					
				}catch (Exception e) {
					logger.severe("Exception while getting FundIndexRateFromFundName for holderId " + sHolderId);
					getExecutionUnitEvent().addException(e);
					return;
				}
				
				//verify that the fund returned by this query is the same fund as returned by the prev query that returned all the Policies 
				//including the funds for the holderId
				
				logger.fine("Comparing query results");
				try {
					ExecutionUnitVerificationHelper.assertEqual(this, fundQuery1, fundQuery2, "Results of queries do not match!", "GetIndexRate and GetPolicy queries returned different results.");
