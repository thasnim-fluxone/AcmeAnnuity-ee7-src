 ...
 */
public class RemoveContactFromAnnuityHolderEU extends AbstractAnnuityStackableExecutionUnit {
		...
		try {
			...
			if (enableVerify){
				...
				if (readResult.getContact() == null){
					throw new ExecutionUnitVerificationException("This holder does not have a contact associated with it.  " +
							"This test should be executed on a a holder that has a contact.  holder id: " + readResult.getId());
				}
			}	
			// now update it
			...
			if (enableVerify){
				...
				if(updateResult.getContact() != null){
					throw new ExecutionUnitVerificationException("Updated the holder contact to null, " +
							"when the holder was re-read, the contact was there.  Contact is: " +updateResult.getContact() +
							" holder id is: " + updateResult.getId() +
							" contact id is: " + updateResult.getContact().getId());
				}
			}
		} catch (Exception e) {
